...
| **ImageGalleryModule** | YES | An image gallery to allow to pick images from a gallery. Can be used with `ImageGalleryField`. | imageGallery | array | `listEndpoint`, `uploadEndpoint`, `searchEndpoint`, `icon`, `buttonTitle`, `uploadTitle` | see ``Ehyiah\QuillJsBundle\DTO\Modules\ImageGalleryModule`` |
