...
When an image is selected within the editor, the `ImageSelectionModule` (enabled by default with `ImageField`) provides additional controls to improve the editing experience:

- **Visual Feedback**: A thick blue border is displayed around the selected image.
- **Insert Paragraph Before**: A button with the icon `¶+` appears at the top-right corner of the image. Clicking it inserts a new empty paragraph immediately before the image.
- **Insert Paragraph After**: A button with the icon `+¶` appears at the bottom-right corner of the image. Clicking it inserts a new empty paragraph immediately after the image.

### Customizing Selection Style

You can customize the appearance of the selection border by passing options to the `ImageSelectionModule` in your form:
        ...
        ->add('content', QuillType::class, [
            ...
            'modules' => [
                new ImageSelectionModule(options: [
                    'borderColor' => '#e63946',
                    'borderWidth' => '6px',
                    'buttonBeforeLabel' => 'Add before',
                    'buttonAfterLabel' => 'Add after',
                    'buttonBeforeTitle' => 'Insert a paragraph before this image',
                    'buttonAfterTitle' => 'Insert a paragraph after this image',
                ]),
            ],
        ])
    ;
...
This makes it easier to add text around images without having to manually manage the cursor position in complex layouts.
