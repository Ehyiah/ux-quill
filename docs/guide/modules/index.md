...
| **TemplatesModule**  |      YES      | Adds a Templates button to the toolbar. Clicking it opens a dropdown of predefined HTML templates that can be inserted at the cursor position. Use with `TemplateField`. | templates | array | array of ``['label' => string, 'content' => string]`` | ``[]`` |
