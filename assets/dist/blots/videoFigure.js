...
class VideoFigure extends BlockEmbed {
  ...
  static value(node) {
    ...
    let caption = (figcaption == null || (_figcaption$textConte = figcaption.textContent) == null ? void 0 : _figcaption$textConte.trim()) || null;
    if (figcaption && figcaption.style.display === 'none') {
      caption = null;
    }
    return {
      src: iframe == null ? void 0 : iframe.getAttribute('src'),
      title: iframe == null ? void 0 : iframe.getAttribute('title'),
      loading: (iframe == null ? void 0 : iframe.getAttribute('loading')) || 'lazy',
      width: node.style.width || null,
      height: (iframe == null ? void 0 : iframe.style.height) || null,
  ...
  format(name, value) {
    ...
    if (name === 'caption') {
      ...
      if (value) iframe.setAttribute('title', value);else iframe.removeAttribute('title');
    } else if (name === 'loading') {
      if (value && value !== 'lazy') iframe.setAttribute('loading', value);else iframe.setAttribute('loading', 'lazy');
    } else if (name === 'width') {
      if (value) {
      ...
      if (value) iframe.style.height = value;else iframe.style.height = '';
    } else if (name === 'style') {
      if (value) {
...
VideoFigure.blotName = 'video';
VideoFigure.tagName = 'figure';
VideoFigure.className = 'ql-video-figure';
