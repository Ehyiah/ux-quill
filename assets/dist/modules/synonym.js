class SynonymModule {
  ...
  async fetchAndUpdateSynonyms(word, synonyms, input, popup) {
    if (!word) return;
    const url = `https://api.conceptnet.io/query?node=/c/${this.lang}/${encodeURIComponent(word)}&rel=/r/Synonym&limit=20`;
    let data;
    try {
      const res = await fetch(url);
      data = await res.json();
