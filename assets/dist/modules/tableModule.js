import { TableCellBlot } from "../blots/tableBlots.js";
...
export class TableModule {
  ...
  constructor(quill, options) {
    ...
    if (this.options.showUI) {
      this.tableUI = new TableUI(this.quill, this, {
    ...
    this.quill.keyboard.addBinding({
      ...
      handler: range => {
        ...
        const nextCell = cells.find(c => {
          const formats = TableCellBlot.formats(c.domNode);
          return parseInt(formats.row || '0', 10) === nextRow && parseInt(formats.col || '0', 10) === nextCol;
        });
        if (nextCell) {
    ...
    this.quill.keyboard.addBinding({
      ...
      handler: range => {
        ...
        const prevCell = cells.find(c => {
          const formats = TableCellBlot.formats(c.domNode);
          return parseInt(formats.row || '0', 10) === prevRow && parseInt(formats.col || '0', 10) === prevCol;
        });
        if (prevCell) {
  ...
  getCellPosition() {
    const cell = this.getCell();
    if (!cell) return null;
    const formats = TableCellBlot.formats(cell.domNode);
  ...
  insertRowAbove() {
    ...
    cells.forEach(c => {
      ...
      if (row >= targetRow) {
        ...
        if (c.parent && c.parent.statics.blotName === 'table-row') {
          c.parent.domNode.setAttribute('data-row', (row + 1).toString());
        }
      }
    });
    ...
    // Create new row
    const scroll = this.quill.scroll;
    const newRowBlot = scroll.create('table-row', {
  ...
  insertColumnLeft() {
    ...
    const tbody = table.children.head;
    if (!tbody) return;
    tbody.children.forEach(rowBlot => {
