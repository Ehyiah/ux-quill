...
export default class VideoSelection {
    ...
    private selectedFigure: HTMLFigureElement | null = null;
    ...
    private getIframe(): HTMLIFrameElement | null {
        return this.selectedFigure?.querySelector('iframe') || null;
    }
...
`;
    ...
    private handleClick(e: MouseEvent): void {
        ...
        const figure = target.closest<HTMLFigureElement>(VIDEO_SELECTOR);
    ...
    private selectVideo(figure: HTMLFigureElement): void {
    ...
    private setupToolbar(): void {
        ...
        this.addSection(this.options.sectionLabels?.video, (container) => {
            ...
            btnTitle.onclick = (e) => { e.stopPropagation(); this.showTitleInput(); };
            ...
            btnLoading.onclick = (e) => { e.stopPropagation(); this.toggleLoading(); };
    ...
    private showTitleInput(): void {
        ...
        input.onkeydown = (e) => {
            if (e.key === 'Enter') { this.setTitle(input.value); this.hideInputBar(); }
    ...
    private showCaptionInput(): void {
        ...
        input.onkeydown = (e) => {
            if (e.key === 'Enter') { this.setCaption(input.value); this.hideInputBar(); }
            else if (e.key === 'Escape') { this.hideInputBar(); }
        };
    ...
    private updateLoadingButton(): void {
        if (!this.toolbar) return;
        const btn = this.toolbar.querySelector('[data-action="loading"]') as HTMLButtonElement | null;
        if (!btn) return;
        const iframe = this.getIframe();
    ...
    private updateActiveButtons(): void {
        ...
        this.toolbar.querySelectorAll('button[data-size]').forEach(btn => {
            ...
            if (s && this.selectedFigure!.style.width === s) btn.classList.add('active');
            else btn.classList.remove('active');
        });
    }
    ...
    private handleMouseDown = (e: MouseEvent): void => {
        ...
        const rect = this.selectedFigure ? this.selectedFigure.getBoundingClientRect() : { width: 0, height: 0 };
