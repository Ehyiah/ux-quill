export function expandWordSelection(
  textOrCharGetter: string | ((i: number) => string),
  index: number,
  length: number
): { index: number; length: number } {
  const getChar = typeof textOrCharGetter === 'function'
    ? textOrCharGetter
    : (i: number) => (i >= 0 && i < textOrCharGetter.length ? textOrCharGetter[i] : '');
...
export function getSingleWordRange(
  text: string,
  selIndex: number,
  selLength: number
