...
export class TablePicker {
    ...
    constructor(quill: Quill, tableModule: TableModule) {
        this.quill = quill;
     ...
     */
    private highlightCells(row: number, col: number): void {
        if (!this.grid) return;

        const cells = this.grid.querySelectorAll('.ql-table-picker-cell');
        cells.forEach((cell) => {
            const cellRow = parseInt((cell as HTMLElement).dataset.row || '0', 10);
            const cellCol = parseInt((cell as HTMLElement).dataset.col || '0', 10);
     ...
     * Show the picker
     */
    showPicker(button: HTMLElement): void {
        if (!this.picker) {
            this.createPicker();
        }
