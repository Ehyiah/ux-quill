...
class QuillType extends AbstractType
{
    public function buildView(FormView $view, FormInterface $form, array $options): void
    {
        ...
        foreach ($fields as $field) {
            if (in_array('emoji', $field, true) && !in_array('emoji-toolbar', array_column($modules, 'name'), true)) {
                $modules[] = new EmojiModule();
            }
        }
        foreach ($fields as $field) {
            if (in_array('image', $field, true) && !in_array('resize', array_column($modules, 'name'), true)) {
